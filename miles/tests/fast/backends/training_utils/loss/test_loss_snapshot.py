...
def run_loss_function_dispatcher(args, parallel_state, inputs):
    ...
    if args.calculate_per_token_loss or getattr(args, "policy_loss_agg_mode", None) == "token-mean":
        global_token_count = sum(loss_mask.sum() for loss_mask in batch["loss_masks"])
