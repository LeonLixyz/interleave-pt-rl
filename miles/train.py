...
async def train(args):
    ...
    async def save(rollout_id):
        ...
        if args.rollout_global_dataset:
            rollout_state = await rollout_manager.checkpoint_state.remote(
                rollout_id
            )
        if (not args.use_critic) or (rollout_id >= args.num_critic_only_steps):
            ...
            if args.train_backend == "fsdp":
                save_kwargs["rollout_state"] = rollout_state
            await actor_model.save_model(rollout_id, **save_kwargs)
        if args.use_critic:
            await critic_model.save_model(
                rollout_id,
                force_sync=rollout_id == args.num_rollout - 1,
            )
        if args.rollout_global_dataset and args.train_backend != "fsdp":
    ...
    # W&B shared-mode workers must flush before the primary driver calls
    # finish_tracking() in __main__.  Otherwise a short resumed process can
    # finish the run while its final training event is still buffered.
