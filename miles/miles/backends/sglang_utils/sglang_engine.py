...
def _normalize_reported_dtype(value) -> str:
    return str(value).removeprefix("torch.").lower()


def _validate_actual_server_runtime(actual: dict, expected: dict) -> dict[str, object]:
    """Validate the running SGLang server, not only its launch arguments."""

    expected_dtype = _normalize_reported_dtype(expected.get("dtype"))
    actual_dtype = _normalize_reported_dtype(actual.get("dtype"))
    if actual_dtype != expected_dtype:
        raise RuntimeError(
            "SGLang runtime dtype mismatch: "
            f"expected={expected_dtype!r} actual={actual_dtype!r}"
        )

    expected_context = expected.get("context_length")
    actual_context = actual.get("context_length")
    if (
        isinstance(expected_context, bool)
        or not isinstance(expected_context, int)
        or expected_context <= 0
    ):
        raise RuntimeError(
            "SGLang expected context_length must be an explicit positive integer"
        )
    if isinstance(actual_context, bool) or not isinstance(actual_context, int):
        raise RuntimeError(
            "SGLang /server_info did not report an integer context_length: "
            f"{actual_context!r}"
        )
    if actual_context != expected_context:
        raise RuntimeError(
            "SGLang runtime context mismatch: "
    ...
    return {
        ...
        "server_version": actual.get("version"),
    }


def _fetch_actual_server_info(server_host: str, server_port: int) -> dict:
    ...
    for endpoint in ("server_info", "get_server_info"):
        try:
            ...
            if not isinstance(value, dict):
                raise RuntimeError(
                    f"SGLang /{endpoint} returned {type(value).__name__}, expected object"
                )
            return value
        except Exception as exc:  # try the legacy endpoint before failing
            last_error = exc
    raise RuntimeError("Unable to read actual SGLang server configuration") from last_error


def _record_precision_gate_sglang_evidence(
    *,
    engine_rank: int,
    worker_type: str,
    expected: dict,
    ...
    """Persist authenticated live-server evidence for the Modal resume gate."""
    ...
    if leg not in {"1", "2"} or not artifact_root:
        raise RuntimeError(
            "Precision-gate SGLang evidence requires leg 1/2 and an artifact root"
        )

    core = {
        "schema": "chess-rl-miles-sglang-runtime-evidence-v1",
        "leg": int(leg),
        "engine_rank": int(engine_rank),
        "worker_type": str(worker_type),
        "expected": {
            "dtype": _normalize_reported_dtype(expected.get("dtype")),
            "context_length": expected.get("context_length"),
        },
        "actual": dict(verified),
    }
    payload = {**core, "evidence_sha256": _canonical_json_sha256(core)}
    output = (
        Path(artifact_root)
        / "precision_gate"
        / "sglang_runtime"
        / f"leg_{leg}_engine_{engine_rank}_{worker_type}.json"
    )
...
class SGLangEngine(RayActor):
    ...
    def init(
        ...
        if self.base_gpu_id is not None and not os.environ.get("CUDA_VISIBLE_DEVICES"):
            ...
            if gpus_per_engine == 1:
                ...
                logger.info(
                    "Set CUDA_VISIBLE_DEVICES=%s for SGLang rank %s",
        ...
        if self.args.rollout_external:
            actual_server_args = self._init_external(
                server_args_dict,
                external_engine_need_check_fields=external_engine_need_check_fields,
            )
        else:
            actual_server_args = self._init_normal(server_args_dict)

        if self.node_rank == 0:
            verified_runtime = _validate_actual_server_runtime(
                actual_server_args,
                server_args_dict,
            )
            _record_precision_gate_sglang_evidence(
                engine_rank=self.rank,
                worker_type=self.worker_type,
                expected=server_args_dict,
                verified=verified_runtime,
            )
            logger.info(
                "Verified actual SGLang runtime contract: dtype=%s context_length=%s",
                verified_runtime["dtype"],
                verified_runtime["context_length"],
            )
    ...
    def _init_external(self, expect_server_args, external_engine_need_check_fields):
        ...
        actual_server_args = _fetch_actual_server_info(
            self.server_host,
            self.server_port,
        )
        _sanity_check_server_args(actual_server_args, expect_server_args)
        return actual_server_args
    ...
    def _init_normal(self, server_args_dict):
        ...
        if self.node_rank == 0 and self.router_ip and self.router_port:
            ...
            response.raise_for_status()
        if self.node_rank == 0:
            return _fetch_actual_server_info(
                self.server_host,
                self.server_port,
            )
        return {}
    ...
    def _weight_update_session_request(self, endpoint: str, payload: dict):
        """Call a transactional weight-update endpoint when the server supports it.

        SGLang versions before the begin/update/end protocol do not expose
        these routes. Cache a 404/405 as a legacy capability result so their
        non-transactional update endpoints continue to work unchanged.
        """
        if getattr(self, "_weight_update_sessions_supported", None) is False:
            return {
                "success": True,
                "message": "Legacy SGLang server does not expose transactional weight updates.",
        ...
        try:
            ...
            can_fall_back_to_legacy = (
                endpoint == "begin_weight_update"
                and getattr(self, "_weight_update_sessions_supported", None) is not True
                and status_code in (404, 405)
            )
            if not can_fall_back_to_legacy:
                raise
            self._weight_update_sessions_supported = False
            logger.warning(
                "SGLang endpoint /%s is unavailable (HTTP %s); using the legacy non-transactional weight-update protocol.",
                endpoint,
                status_code,
            )
            return {
        ...
        self._weight_update_sessions_supported = True
        return result

    def begin_weight_update(self, selector: str = "all"):
        """Open a weight-update transaction on current SGLang servers."""
        return self._weight_update_session_request("begin_weight_update", {"selector": selector})

    def end_weight_update(self):
        """Finalize and close the current SGLang weight-update transaction."""
...
def _compute_server_args(
    ...
    # The canonical HF checkpoint may contain FP32 training weights. SGLang
    # should load that same checkpoint and cast it in memory for inference;
    # a separately persisted BF16 export is unnecessary.
    rollout_dtype = getattr(args, "sglang_dtype", None)
    if rollout_dtype not in {"bfloat16", "float16"}:
        raise RuntimeError(
            "SGLang inference dtype must be explicit: --sglang-dtype must be bfloat16 or float16"
        )
    kwargs["dtype"] = rollout_dtype
