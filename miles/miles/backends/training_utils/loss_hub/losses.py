...
def _require_finite_active_tokens(
    value: torch.Tensor,
    active_tokens: torch.Tensor,
    *,
    name: str,
) -> None:
    """Reject NaN/Inf on supervised tokens before any masking operation."""

    if value.shape != active_tokens.shape:
        raise RuntimeError(
            f"active-token finite check shape mismatch for {name}: "
            f"value={tuple(value.shape)} mask={tuple(active_tokens.shape)}"
        )
    active_value = value[active_tokens]
    if active_value.numel() == 0:
        raise RuntimeError(
            f"policy loss received no active supervised tokens while checking {name}"
        )
    if not bool(torch.isfinite(active_value).all().item()):
        nonfinite = int((~torch.isfinite(active_value)).sum().item())
        raise RuntimeError(
            f"nonfinite {name} on {nonfinite} active supervised token(s)"
        )


def _use_token_mean_loss(args: Namespace) -> bool:
    """Use one definition for every policy-loss reducer and metric."""

    return bool(
        args.calculate_per_token_loss
        or getattr(args, "policy_loss_agg_mode", None) == "token-mean"
    )
...
def policy_loss_function(
    ...
    use_token_mean_loss = _use_token_mean_loss(args)
    ...
    for name, value in (
        ...
        _require_finite_active_tokens(value, active_tokens, name=name)
    ppo_kl = torch.where(
        active_tokens,
        ppo_kl,
        ppo_kl.new_zeros(()),
    ...
    if args.advantage_estimator == "cispo":
        pg_loss, pg_clipfrac = compute_cispo_policy_loss(
            ppo_kl,
            log_probs,
            advantages,
            args.cispo_clip_min,
    ...
    if args.custom_pg_loss_reducer_function_path is not None:
        ...
        pg_loss_reducer = custom_pg_loss_reducer_func(
            total_lengths, response_lengths, pg_loss_masks, use_token_mean_loss
        )
    else:
    ...
    ess_ratio_sum = compute_ess_ratio_contribution(
        ...
        max_seq_lens=max_seq_lens,
        calculate_per_token_loss=use_token_mean_loss,
    )
    ...
    if args.use_kl_loss:
        ...
        _require_finite_active_tokens(
            ref_log_probs,
            active_tokens,
            name="reference_log_probs",
        )
        _require_finite_active_tokens(kl, active_tokens, name="reference_kl")
        kl = torch.where(
            active_tokens,
            kl,
            kl.new_zeros(()),
    ...
    if "rollout_log_probs" in batch and batch["rollout_log_probs"]:
        ...
        _require_finite_active_tokens(
            rollout_log_probs,
            active_tokens,
            name="rollout_log_probs",
        )
        abs_diff = (train_scored_log_probs - rollout_log_probs).abs()
        _require_finite_active_tokens(
            abs_diff,
            active_tokens,
            name="train_rollout_logprob_abs_diff",
        )
