...
def peek_supervised_token_count(data_iterator: DataIterator, num_microbatches: int) -> torch.Tensor:
    """Count upcoming supervised tokens without changing iterator position."""
    if num_microbatches <= 0:
        raise ValueError("num_microbatches must be positive")
    start_offset = data_iterator.offset
    total = None
    try:
        for _ in range(num_microbatches):
            count_batch = data_iterator.get_next(["loss_masks"])
            microbatch_total = sum(loss_mask.sum() for loss_mask in count_batch["loss_masks"])
            total = microbatch_total if total is None else total + microbatch_total
    finally:
        data_iterator.offset = start_offset
