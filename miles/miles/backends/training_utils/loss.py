...
def loss_function(
    ...
    global_token_count: torch.Tensor | None = None,
    ...
    Args:
        ...
        global_token_count: DP-global number of supervised tokens in the
            complete optimizer update. Required by the non-Megatron
            token-mean path so all micro-batches share one denominator.
    ...
    # Logging/accounting must report real supervised tokens.  Giving an
    # all-zero row one synthetic token hides invalid rows and disagrees with
    # the denominator used by global token-mean training.
    ...
    if not use_token_mean_loss:
        ...
        if apply_megatron_loss_scaling:
            ...
            if global_token_count is None:
                raise RuntimeError(
                    "Global token-mean loss requires the DP-global supervised-token count for the complete "
                    "optimizer update; a per-microbatch denominator changes the gradient when packing differs"
                )
            global_num_tokens = global_token_count.to(device=loss.device, dtype=loss.dtype)
            # FSDP2 averages replicated-DP gradients by default. Scale each
            # rank's local token sum by DP size so the post-AVG gradient is
            # sum(global token gradients) / global_num_tokens.
            loss = (
                loss
                / torch.clamp_min(global_num_tokens, 1)
                * parallel_state.intra_dp.size
            )
