...
from .precision import assert_rollout_tensor_dtypes, cast_tensor_for_rollout
...
class UpdateWeight(abc.ABC):
    ...
    def update_weights(self) -> None:
        ...
        for name, param in self.model.state_dict().items():
            # Training keeps FP32 master parameters. SGLang executes in BF16
            # (or FP16 when requested), so cast the transient synchronized
            # tensor without mutating or exporting the training checkpoint.
            param = cast_tensor_for_rollout(param, self.args)
    ...
    def wait_and_update_bucket_weights(self, bucket):
        ...
        assert_rollout_tensor_dtypes(bucket, self.args)
