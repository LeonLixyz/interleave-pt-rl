...
def init_wandb_primary(args):
    ...
    requested_run_id = getattr(args, "wandb_run_id", None)
    if requested_run_id:
    ...
    _record_precision_gate_wandb(args, group=group, run_name=run_name)


def _record_precision_gate_wandb(args, *, group: str, run_name: str) -> None:
    leg = os.environ.get("CHESS_RL_MILES_PRECISION_GATE_LEG")
    artifact_root = os.environ.get("CHESS_RL_MILES_ARTIFACT_ROOT")
    if leg is None:
        return
    if leg not in {"1", "2"} or not artifact_root or wandb.run is None:
        raise RuntimeError(
            "Precision-gate W&B evidence requires leg 1/2, artifact root, and a live run"
        )
    core = {
        "schema": "miles-precision-gate-wandb-v1",
        "leg": int(leg),
        "entity": args.wandb_team,
        "project": args.wandb_project,
        "group": group,
    ...
    output = (
        Path(artifact_root)
        / "precision_gate"
        / f"leg_{leg}_wandb.json"
    )
    output.parent.mkdir(parents=True, exist_ok=True)
    temporary = output.with_name(f".{output.name}.{os.getpid()}.tmp")
    with temporary.open("w", encoding="utf-8") as handle:
        json.dump(
            {**core, "evidence_sha256": digest},
            handle,
            indent=2,
            sort_keys=True,
        )
