...
def _board_after_setup_move(fen: str, setup_uci: str = ""):
    """Return the position in which the model makes its first move."""
    if not fen:
        return None
    try:
        import chess
        ...
        if setup_uci:
            move = chess.Move.from_uci(setup_uci)
...
def _check_move_legality(
    fen: str,
    uci_move: str,
    setup_uci: str = "",
) -> float:
    """Check legality in the post-setup puzzle position."""
    if not fen or not uci_move or not setup_uci:
        return 0.0
    try:
        import chess

        board = _board_after_setup_move(fen, setup_uci)
        if board is None:
            return 0.0
...
def _setup_move_uci(extra_info) -> str:
    if not isinstance(extra_info, dict):
        return ""
    moves = str(extra_info.get("Moves") or extra_info.get("moves") or "").split()
    moves_setup = moves[0] if moves else ""
    explicit = str(extra_info.get("first_move_uci") or "").strip()
    if explicit and moves_setup and explicit != moves_setup:
        return ""
    return explicit or moves_setup


def _side_to_move_after_setup(fen: str, setup_uci: str) -> str | None:
    if not setup_uci:
        return None
    board = _board_after_setup_move(fen, setup_uci)
    if board is None:
...
def compute_score_batch(data_sources, solution_strs, ground_truths, extra_infos):
    ...
    for data_source, solution_str, ground_truth, extra_info in zip(
        ...
        fen = str(extra_info.get('FEN') or extra_info.get('fen') or '')
        setup_uci = _setup_move_uci(extra_info)
        model_side = _side_to_move_after_setup(fen, setup_uci)
        ...
        if not extracted_moves:
            ...
            for i, move in enumerate(extracted_moves):
                if move is None:
                    ...
                    try:
                        stripped = str(move).rstrip('+#').strip()
                        if stripped in {'O-O', 'O-O-O'} and model_side is None:
                            raise ValueError(
                                "Cannot convert castling without a valid puzzle position"
                            )
                        uci = lan_to_uci(move, side_to_move=model_side or 'white')
                    except ValueError:
                        uci = ''
        ...
        first_move_legality_score = _check_move_legality(
            fen,
            first_pred_uci,
            setup_uci,
        )
