...
from chess_rl_miles.scripts import modal_v2r4d_gate as gate
...
def test_contract_is_fresh_uniform_six_cell_grid():
    ...
    assert contract["version"] == "v2r4d_production_gate_20260730"
...
def test_direct_file_launcher_bootstraps_project_before_package_import():
    source = Path(gate.__file__).read_text()
    ...
    first_package_import = source.index("from chess_rl_miles.")
    assert bootstrap < insertion < first_package_import


def test_isolated_modal_style_import_proves_bootstrap_sensitivity(tmp_path):
    source = Path(gate.__file__).read_text()
    project_link = tmp_path / "chess-rl-miles"
    project_link.symlink_to(gate.base.PROJECT_LOCAL, target_is_directory=True)
    launcher = tmp_path / "modal_v2r4d_gate.py"
    ...
    start = source.index("_REMOTE_PROJECT_DIRS = (")
    end = source.index("from chess_rl_miles.provenance", start)
    broken = tmp_path / "modal_v2r4d_gate_without_bootstrap.py"
    broken.write_text(source[:start] + source[end:])
    rejected = subprocess.run(
        [sys.executable, "-I", str(broken)],
        cwd=tmp_path,
        text=True,
        capture_output=True,
        timeout=30,
    ...
    assert "No module named 'chess_rl_miles'" in rejected.stderr


def test_successor_binds_zero_outcome_incident_and_legacy_prompt_bytes():
    contract = gate._contract_static()
    prompt = contract["prompt_manifest"]
    incident = contract["predecessor_incident"]

    assert prompt["source_schema"] == (
        "interleaved-v2r4c-fresh-prompt-batches-v1"
    )
    assert prompt["source_version"] == (
        "v2r4c_production_gate_20260730"
    )
    assert (
        prompt["reused_after_zero_outcome_preflight_failure"] is True
    )
    assert "/v2r4c_production_gate_20260730/" in prompt["path"]
    assert incident == gate.PREDECESSOR_INCIDENT
    assert incident["report_sha256"] == (
        "2a2eaeb867d73366b69377e6f965a925eff0c3db8bd8bae881ec4d035e3ec5f0"
    )
    assert incident["remote_call_children"] == 0
    assert incident["gpu_roots_created"] == 0
    assert incident["outcome_exposure"] is False
    assert incident["prompt_artifact_reuse_authorized"] is True


def test_import_probe_is_cpu_only_and_has_no_mutation_surface():
    source = Path(gate.__file__).read_text()
    tree = ast.parse(source)
    node = next(
        value
        ...
        and value.name == "v2r4d_import_probe"
    )
    decorator = next(
        value
    ...
    keywords = {value.arg for value in decorator.keywords}
    assert keywords == {"cpu", "memory", "timeout"}
    probe_source = _function_source("v2r4d_import_probe")
    assert "Volume" not in probe_source
    assert "contract" not in probe_source
    assert "ledger" not in probe_source
    assert "gpu_requested" in probe_source


def test_contract_import_probe_binds_current_launcher_digest():
    result = {
        "schema": "interleaved-v2r4d-import-probe-v1",
        "version": gate.VERSION,
        "launcher_path": "/root/modal_v2r4d_gate.py",
        "launcher_sha256": gate.shared._sha256(Path(gate.__file__)),
    ...
    evidence = {
        "report_path": gate.IMPORT_PROBE_PATH,
        "report_sha256": "1" * 64,
    ...
    gate._validate_import_probe_contract(evidence)

    tampered = json.loads(json.dumps(evidence))
    tampered["result"]["launcher_sha256"] = "3" * 64
    with pytest.raises(ValueError, match="import-probe"):
        gate._validate_import_probe_contract(tampered)


def test_predecessor_absence_rejects_any_late_root_or_result(tmp_path):
    checkpoint_mount = tmp_path / "checkpoints"
    results_mount = tmp_path / "results"
    checkpoint_mount.mkdir()
    results_mount.mkdir()
    assert gate._validate_predecessor_absence(
        checkpoint_mount=checkpoint_mount,
        results_mount=results_mount,
    )["predecessor_root_count"] == 7

    old_root = (
        checkpoint_mount
        / "chess-rl-miles-interleave"
        / gate.PREDECESSOR_RUN_NAMES[0]
    )
    old_root.mkdir(parents=True)
    with pytest.raises(FileExistsError, match="predecessor roots"):
        gate._validate_predecessor_absence(
            checkpoint_mount=checkpoint_mount,
            results_mount=results_mount,
        )
    old_root.rmdir()

    old_result = results_mount / "audit" / "v2r4c-result.json"
    old_result.parent.mkdir()
    old_result.write_text("{}")
    with pytest.raises(FileExistsError, match="result artifacts"):
        gate._validate_predecessor_absence(
            checkpoint_mount=checkpoint_mount,
            results_mount=results_mount,
        )
...
def test_gate_runtime_is_blinded_and_watchdogs_are_suppressed():
    ...
    source = _function_source("v2r4d_gate_rollout")
...
def test_gpu_functions_have_hard_timeout_and_no_automatic_retries():
    source = Path(gate.__file__).read_text()
    tree = ast.parse(source)
    for name in ("v2r4d_canary", "v2r4d_gate_rollout"):
        node = next(
            value
            for value in tree.body
            if isinstance(value, ast.FunctionDef)
...
def _make_strict_inventory(tmp_path: Path, rollouts: int) -> tuple[Path, str]:
    ...
    provenance.mkdir()
    (run_root / "_V2R4D_GATE_INTENT.json").write_text("{}")
...
def test_all_zero_contract_binding_is_rejected_before_ledger_code(
    ...
    main_source = _function_source("v2r4d_main")
...
def test_binding_is_the_only_v2r4d_project_manifest_exclusion():
