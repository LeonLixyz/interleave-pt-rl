"""Corrected, fresh-prompt v2r4c rollout gate and exact-once launcher."""
...
from chess_rl_miles.v2r4c_contract_binding import (
...
VERSION = "v2r4c_production_gate_20260730"
CONTRACT_SCHEMA = (
    "interleaved-v2r4c-production-gate-runtime-contract-v1"
)
...
CONTRACT_PATH = (
    ...
    "v2r4c_production_gate_20260730/runtime_contract.json"
)
CANARY_PATH = (
    "/data/50m_interleaved_mix10b_sft90k_v2r1_clean_verify_gate/"
...
CANARY_RUN_NAME = "v2r4c-runtime-canary-s6000-seed13477620"
CELL_TIMEOUT_SECONDS = 2 * 60 * 60
ROUTER_HEALTH_INTERVAL_SECONDS = 1e18
CANARY_LEDGER_DEFAULT = (
    "INTERLEAVED_V2R4C_CANARY_LAUNCH_LEDGER.json"
)
GRID_LEDGER_DEFAULT = "INTERLEAVED_V2R4C_GATE_LAUNCH_LEDGER.json"
FINALIZER_PATH = "Eval/finalize_v2r4c_gate.py"
REUSED_FINALIZER_PATH = "Eval/finalize_v2r4_gate.py"
FINALIZER_TEST_PATH = "Eval/tests/test_finalize_v2r4c_gate.py"
...
def _run_name(candidate_step: int, batch_label: str) -> str:
    return (
        f"v2r4c-gate-w190-s{candidate_step}-"
...
def _require_contract_digest(requested_sha256: str) -> None:
    if (
        ...
        raise ValueError(
            "v2r4c launch requires a non-placeholder frozen contract SHA256"
        )
...
def _validate_finalizer_contract(value: object) -> None:
    if not isinstance(value, dict) or set(value) != {
        ...
        raise ValueError("v2r4c contract lacks the exact finalizer binding")
    if (
        value.get("path") != FINALIZER_PATH
        or not re.fullmatch(r"[0-9a-f]{64}", str(value.get("sha256", "")))
    ):
        raise ValueError("v2r4c finalizer source binding drifted")
    for field, expected_path in (
        ...
        if (
            ...
            raise ValueError(f"v2r4c finalizer {field} binding drifted")
...
def _validate_analysis_contract(value: object) -> None:
    if not isinstance(value, dict) or set(value) != {
        ...
        raise ValueError("v2r4c contract lacks the exact analyzer binding")
    if (
        value.get("path") != ANALYZER_PATH
        or not re.fullmatch(r"[0-9a-f]{64}", str(value.get("sha256", "")))
    ):
        raise ValueError("v2r4c analyzer source binding drifted")
    for field, expected_path in (
        ...
        if (
            ...
            raise ValueError(f"v2r4c analyzer {field} binding drifted")
...
def _load_contract(requested_sha256: str) -> dict[str, object]:
    ...
    if not re.fullmatch(
        ...
        raise RuntimeError("v2r4c contract file binding is not frozen")
    path = Path(CONTRACT_PATH)
    if (
        not path.is_file()
        or shared._sha256(path) != EXPECTED_CONTRACT_FILE_SHA256
    ):
        raise ValueError("v2r4c runtime-contract file drifted")
    contract = json.loads(path.read_text())
    if not isinstance(contract, dict):
        raise ValueError("v2r4c runtime contract is not an object")
    ...
    if (
        ...
        raise ValueError("v2r4c runtime-contract self-hash drifted")
    for key, expected in _contract_static().items():
        if contract.get(key) != expected:
            raise ValueError(f"v2r4c contract field drifted: {key}")

    sources = contract.get("sources")
    if not isinstance(sources, dict):
        raise ValueError("v2r4c contract lacks source identities")
    ...
    if sources.get("chess_rl_miles") != actual_project:
        raise ValueError("v2r4c project source identity drifted")
    if sources.get("miles") != actual_miles:
        raise ValueError("v2r4c Miles source identity drifted")
    ...
    if (
        ...
        raise ValueError(
            "v2r4c contract lacks frozen plan/analysis/evaluator identities"
        )
...
def _load_prompt_manifest() -> dict[str, object]:
    ...
    if (
        ...
        raise ValueError("v2r4c fresh prompt manifest file drifted")
    payload = json.loads(path.read_text())
    if not isinstance(payload, dict):
        raise ValueError("v2r4c fresh prompt manifest is not an object")
    ...
    if (
        ...
        raise ValueError("v2r4c fresh prompt manifest self-hash drifted")
    ...
    if (
        ...
        raise ValueError("v2r4c prompt-set disjointness proof drifted")
    expected_sets = {
        "A": {
    ...
    for label, expected in expected_sets.items():
        ...
        if not isinstance(observed, dict):
            raise ValueError(f"v2r4c prompt set is missing: {label}")
        normalized = {
        ...
        if normalized != expected:
            raise ValueError(f"v2r4c prompt set drifted: {label}")
    prompt_sets = {
    ...
    if (
        ...
        raise ValueError("v2r4c direct prompt intersection check failed")
...
def _validate_prompt_data_files() -> None:
    ...
    for path, digest, rows in expected:
        if not path.is_file() or shared._sha256(path) != digest:
            raise ValueError(f"v2r4c prompt parquet drifted: {path}")
        if pq.ParquetFile(path).metadata.num_rows != rows:
            raise ValueError(f"v2r4c prompt row count drifted: {path}")
...
def _validate_canary(
    ...
    *,
    ...
    _strict_inventory(
        run_root,
        expected_rollouts=1,
        intent_filename="_V2R4C_CANARY_INTENT.json",
        launch_manifest=launch_manifest,
        success_marker_filename=(
            "_V2R4C_CANARY_SUCCESS.json"
...
def v2r4c_preflight(
    ...
    if existing_grid:
        raise FileExistsError(
            "v2r4c canonical grid roots already exist: "
            + ", ".join(existing_grid)
        )
    ...
    if phase == "grid":
        marker = canary_root / "_V2R4C_CANARY_SUCCESS.json"
        ...
        if (
            ...
            raise ValueError("v2r4c canary success marker drifted")
        provenance = canary_success.get("provenance")
        if not isinstance(provenance, dict):
            raise ValueError("v2r4c canary provenance is missing")
        ...
        if canary_success.get("artifact") != authenticated_artifact:
            raise ValueError("v2r4c canary artifact binding drifted")
    ray_worker_environment = shared._verify_ray_worker_gate_environment(
        "v2r4c-ray-env-preflight"
    )
    return {
...
def _run_rollout_command(
    *,
    ...
    try:
        ...
        raise RuntimeError(
            f"v2r4c rollout timed out for {run_name}"
        ) from exc
    if result.returncode:
        base.ckpt_vol.commit()
        raise RuntimeError(
            f"v2r4c rollout failed for {run_name}: "
...
def v2r4c_canary(contract_sha256: str) -> dict[str, object]:
    base.data_vol.reload()
    base.ckpt_vol.reload()
    ...
    if not source.is_file() or shared._sha256(source) != CANARY_SHA256:
        raise ValueError("v2r4c canary data drifted")
    import pyarrow.parquet as pq

    if pq.ParquetFile(source).metadata.num_rows != CANARY_ROWS:
        raise ValueError("v2r4c canary row count drifted")
    ...
    if (
        ...
        raise ValueError("v2r4c canary checkpoint drifted")
    ...
    command = shared.build_train_command(
        ...
        wandb_group="v2r4c_runtime_canary",
    ...
    shared._atomic_json(
        run_root / "_V2R4C_CANARY_INTENT.json",
        {
            "schema": "interleaved-v2r4c-canary-intent-v1",
            "version": VERSION,
            "contract_sha256": contract_sha256,
            "run_name": CANARY_RUN_NAME,
    ...
    provenance = write_run_provenance(
        run_root=run_root,
        identity={
            "kind": "chess_rl_miles_v2r4c_runtime_canary",
            "version": VERSION,
            "contract_sha256": contract_sha256,
            "contract_file_sha256": contract["contract_file_sha256"],
            "candidate": {
    ...
    base.ckpt_vol.commit()
    print("[v2r4c-canary] " + " ".join(command), flush=True)
    ...
    core = {
        "schema": "interleaved-v2r4c-canary-success-v1",
        "version": VERSION,
        "contract_sha256": contract_sha256,
        "contract_file_sha256": contract["contract_file_sha256"],
        "run_name": CANARY_RUN_NAME,
    ...
    shared._atomic_json(
        run_root / "_V2R4C_CANARY_SUCCESS.json", success
    )
    base.ckpt_vol.commit()
    return success
...
def v2r4c_gate_rollout(
    *,
    ...
    if (
        ...
        raise ValueError("v2r4c cell is outside the frozen grid")
    base.data_vol.reload()
    base.ckpt_vol.reload()
    shared.pretrain_ckpt_vol.reload()
    contract = _load_contract(contract_sha256)
    ...
    if (
        ...
        raise ValueError("v2r4c gate lacks an authenticated canary")
    ...
    if len(authorized) != 1:
        raise ValueError("v2r4c contract does not authorize this cell")
    checkpoint = shared._validate_hf_checkpoint(
    ...
    command = shared.build_train_command(
        ...
        eval_interval=0,
        wandb_project="chess_interleave_50m",
        wandb_group="v2r4c_production_gate",
        deterministic_inference=True,
        rollout_only=True,
    ...
    run_root.mkdir(parents=True, exist_ok=False)
    shared._atomic_json(
        run_root / "_V2R4C_GATE_INTENT.json",
        {
            "schema": "interleaved-v2r4c-gate-cell-intent-v1",
            "version": VERSION,
            "contract_sha256": contract_sha256,
            "contract_file_sha256": contract["contract_file_sha256"],
            "candidate_step": candidate_step,
    ...
    provenance = write_run_provenance(
        run_root=run_root,
        identity={
            "kind": "chess_rl_miles_v2r4c_production_gate_rollout",
            "version": VERSION,
            "contract_sha256": contract_sha256,
            "contract_file_sha256": contract["contract_file_sha256"],
    ...
    base.ckpt_vol.commit()
    print("[v2r4c-gate] " + " ".join(command), flush=True)
    ...
    core = {
        "schema": "interleaved-v2r4c-gate-cell-success-v1",
        "version": VERSION,
        "contract_sha256": contract_sha256,
        "contract_file_sha256": contract["contract_file_sha256"],
        "run_name": run_name,
    ...
    shared._atomic_json(
        run_root / "_V2R4C_GATE_SUCCESS.json", success
    )
    base.ckpt_vol.commit()
    return success
...
def _load_authenticated_canary_ledger(
    ...
    *,
    ...
    if not isinstance(ledger, dict):
        raise ValueError("v2r4c canary ledger is not an object")
    core = {
        key: value
    ...
    if (
        ...
        != "interleaved-v2r4c-canary-launch-ledger-v1"
        ...
        != "interleaved-v2r4c-gate-preflight-v1"
        ...
        raise ValueError("v2r4c canary launch ledger drifted")
    return ledger
...
def v2r4c_main(
    ...
    if action == "launch-canary":
        ...
        try:
            preflight = v2r4c_preflight.spawn(
        ...
        if (
            ...
            ledger["state"] = "preflight_failed"
            _persist_ledger(path, ledger)
            raise RuntimeError("v2r4c canary preflight failed")
        ledger["preflight"] = result
        ledger["state"] = "canary_launching"
        _persist_ledger(path, ledger)
        try:
            call = v2r4c_canary.spawn(contract_sha256)
        except Exception as exc:
        ...
        _persist_ledger(path, ledger)
        print(f"SPAWNED v2r4c canary: {call.object_id}", flush=True)
        try:
            success = call.get()
        except Exception as exc:
    ...
    if action == "launch-grid":
        ...
        if path.exists():
            raise FileExistsError(path)
        ledger = {
            "schema": "interleaved-v2r4c-gate-launch-ledger-v1",
            "version": VERSION,
            "state": "preflight_launching",
            "contract_sha256": contract_sha256,
        ...
        if (
            ...
            ledger["state"] = "preflight_failed"
            _persist_ledger(path, ledger)
            raise RuntimeError("v2r4c grid preflight failed")
        ledger["preflight"] = result
