#!/bin/bash
# Usage: bash bash_sl.sh [NUM_SHARD]
# Example: bash bash_sl.sh 4   → passes --override data.num_shard=4
NUM_SHARDS=${1:-""}
...
#SBATCH --mem=300G
#SBATCH --job-name=train_${tok}_${cfg_base}_${NUM_SHARDS}
#SBATCH --output=pretrain_slurm_logs/train_${tok}_${cfg_base}_${NUM_SHARDS}_%j.log
...
uv run accelerate launch --num_processes 2 scripts/train/train_hf.py \\
  --config ${config} --auto_resume \\
  ${NUM_SHARDS:+--override data.num_shards=${NUM_SHARDS}}
EOF
  done
done
