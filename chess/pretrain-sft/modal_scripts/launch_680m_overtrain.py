"""
Modal launcher: 3 over-trained 680m pretrains (alpha > 1).

Jobs:
  - 680m_C_6p5e19_alpha1.500  (23.992B tokens)
  - 680m_C_6p5e19_alpha2.000  (31.990B tokens)
  - 680m_C_6p5e19_alpha3.000  (47.985B tokens)
...
JOBS = [
    "config/configs/6p5e19_leon/680m_alpha1.500.yaml",   # 23.992B
    "config/configs/6p5e19_leon/680m_alpha2.000.yaml",   # 31.990B
    "config/configs/6p5e19_leon/680m_alpha3.000.yaml",   # 47.985B
]
...
def main(dry_run: bool = False):
    print(f"Launcher: 680m overtrain sweep ({len(JOBS)} jobs)")
    print(f"GPU: {GPUS_PER_NODE}x {GPU_TYPE}  |  Data: {DATA_DIR}")
    for cfg in JOBS:
    ...
    if dry_run:
        ...
        return
    for cfg in JOBS:
        train_h200.spawn(config=cfg, overrides=_overrides())
        print(f"  SPAWNED: {Path(cfg).stem}")
    print(f"\n{len(JOBS)} jobs spawned (detached).")
