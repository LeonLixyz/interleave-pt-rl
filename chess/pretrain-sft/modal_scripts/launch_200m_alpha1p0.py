"""
Modal launcher: single pretrain run.

Job:
  - 200m_C_6p5e19_alpha1.000  (53.450B tokens)

Mirrors launch_6_pretrains_yellow.py but with one spec.
"""
import os
import sys
...
JOB = "config/configs/6p5e19_leon/200m_alpha1.000.yaml"
...
def main(dry_run: bool = False):
    print(f"Single pretrain launcher: {JOB}")
    print(f"GPU: {GPUS_PER_NODE}x {GPU_TYPE}  |  Data: {DATA_DIR}")
    if dry_run:
        print("(dry-run -- nothing launched)")
        return
    handle = train_h200.spawn(config=JOB, overrides=_overrides())
    print(f"SPAWNED: {Path(JOB).stem}  (detached)")
