"""
Modal launcher: interleave-experiment pretrain leg-2 runs (20m @ C_total=6.5e18).

Arms (one job per invocation via --config):
  - 20m_ilv_C_pt2  control:     alpha1.000@step_60480 -> +40,321 steps, no RL between
  - 20m_ilv_B_pt2  interleaved: SFT->RL2500(step_60480) -> +40,321 steps

Both use skip_shards=27,822 so leg 2 trains on the shard-permutation suffix the
first 60,480 steps never touched. 8x H200, bf16 autocast with fp32 params.

Usage:
  modal run modal_scripts/launch_interleave_pretrains.py --config config/configs/6p5e18_small/20m_ilv_C_pt2.yaml --dry-run
  modal run --detach modal_scripts/launch_interleave_pretrains.py --config config/configs/6p5e18_small/20m_ilv_C_pt2.yaml
"""
import os
...
def _run_training(config: str, num_gpus: int, overrides: list[str] = None):
    ...
    init_ckpt = cfg.get("model", {}).get("pretrained_model")
    if init_ckpt and not Path(init_ckpt, "model.safetensors").exists():
        raise FileNotFoundError(f"init checkpoint not staged: {init_ckpt}")
...
def main(config: str, dry_run: bool = False):
    print(f"Interleave pretrain leg-2 launcher")
    print(f"GPU: {GPUS_PER_NODE}x {GPU_TYPE}  |  Data: {DATA_DIR}  |  Config: {config}")
    if dry_run:
        print("(dry-run -- nothing launched)")
        return
    handle = train_h200.spawn(config=config, overrides=_overrides())
    print(f"SPAWNED: {Path(config).stem}  (function call id: {handle.object_id})")
    print("Monitor: modal app logs pretrain")
