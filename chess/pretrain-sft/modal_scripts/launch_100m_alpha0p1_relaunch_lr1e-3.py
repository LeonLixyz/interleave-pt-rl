"""
Modal launcher: relaunch 100m_C_6p5e18_alpha0.100 with lr=1e-3 override.

Original yaml has lr=2e-3 (the 6p5e18/ dir convention). This launcher overrides
to 1e-3 at runtime to match the 6p5e18_small/ + 6p5e19_leon/ convention.

8x H200, bf16. Modal app name: "pretrain".

Usage:
  modal run modal_scripts/launch_100m_alpha0p1_relaunch_lr1e-3.py --dry-run
  modal run --detach modal_scripts/launch_100m_alpha0p1_relaunch_lr1e-3.py
"""
import os
...
def main(dry_run: bool = False):
    cfg = "config/configs/6p5e18/100m_alpha0.100.yaml"
    overrides = [
    ...
    print(f"Launcher: 100m_alpha0.100 (6p5e18) RELAUNCH with lr=1e-3, eta_min=1e-4 (matches 6p5e18_small/ + 6p5e19_leon/ convention)")
    print(f"  config: {cfg}")
    ...
    print(f"SPAWNED: 100m_alpha0.100 lr=1e-3 (handle={handle.object_id})")
