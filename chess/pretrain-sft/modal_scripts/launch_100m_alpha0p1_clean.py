"""
Modal launcher: 100m_C_6p5e18_alpha0.100 (clean relaunch).

YAML now has lr=1e-3, eta_min=1e-4 baked in directly (no overrides for those).
Worker wipes any pre-existing run dir + does NOT use --auto_resume so we always
start from scratch.

8x H200, bf16. Modal app name: "pretrain".

Usage:
  modal run --detach modal_scripts/launch_100m_alpha0p1_clean.py
"""
import os
...
def main(dry_run: bool = False):
    ...
    print(f"Launcher: 100m_alpha0.100 (6p5e18) clean — yaml has lr=1e-3, eta_min=1e-4")
    print(f"  config: {cfg}")
    ...
    print(f"SPAWNED: 100m_alpha0.100 (handle={handle.object_id})")
