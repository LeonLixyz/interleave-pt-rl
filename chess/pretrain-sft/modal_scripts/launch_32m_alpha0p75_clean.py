"""
Modal launcher: 32m_C_6p5e18_alpha0.750 (clean relaunch).

YAML already has lr=1e-3, eta_min=1e-4. In-worker wipe + no --auto_resume.

8x H200, bf16. Modal app name: "pretrain".

Usage:
  modal run --detach modal_scripts/launch_32m_alpha0p75_clean.py
"""
import os
...
def main(dry_run: bool = False):
    cfg = "config/configs/6p5e18_small/32m_alpha0.750.yaml"
    overrides = [
        "training.gpu_peak_tflops=989",
        "training.cache_size=0",
        "training.mixed_precision=bf16",
    ]
    print(f"Launcher: 32m_alpha0.750 (6p5e18) clean — yaml has lr=1e-3, eta_min=1e-4")
    print(f"  config: {cfg}")
    ...
    print(f"SPAWNED: 32m_alpha0.750 (handle={handle.object_id})")
