...
class HumanGamesEvaluator(BaseEvaluator):
    ...
    def load_data(self, file_path="/scratch/js15262/LLM-Pretraining/data/chess/test/human_games_test_opening.parquet") -> pd.DataFrame:
        """Load parquet file."""
        if file_path.endswith('.parquet'):
            return pd.read_parquet(file_path)
        elif file_path.endswith('.csv'):
...
class PuzzlesEvaluator(BaseEvaluator):
    ...
    def load_data(self, file_path="/scratch/js15262/LLM-Pretraining/data/chess/test/puzzles_test.csv") -> pd.DataFrame:
        """Load parquet file."""
        import chess.pgn
        import io
...
class RandomGamesEvaluator(BaseEvaluator):
    ...
    def load_data(self, file_path="/scratch/js15262/LLM-Pretraining/data/chess/test/random_games_100.parquet") -> pd.DataFrame:
        """Load parquet file."""
        if file_path.endswith('.parquet'):
            return pd.read_parquet(file_path)
        elif file_path.endswith('.csv'):
