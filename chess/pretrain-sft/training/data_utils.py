...
class ShardedPackedTextDataset(Dataset):
    def __init__(self, txt_files: list[str], tokenizer=None, seq_len: int = 512, cache_size: int = 1_000_000, num_shards: int = None, shuffle: bool = False, seed: int = None, skip_shards: int = 0):
        """
        ...
        Args:
            ...
            skip_shards: Drop the first N shards AFTER the seeded shuffle (continue-pretraining
                from where a previous run with the same file list + seed stopped, without
                re-seeing its data)
        """
        self.tok = tokenizer
        self.seq_len = seq_len
        ...
        if len(txt_files) == 1:
            ...
            if isinstance(txt_files, (str, Path)):
                p = Path(txt_files)
                if p.is_dir():
                    # Look for both .txt and .npy files
        ...
        if skip_shards:
            if skip_shards >= len(self.txt_files):
                raise ValueError(
                    f"skip_shards={skip_shards} >= available shards ({len(self.txt_files)})"
                )
            print(f"[dataset] Skipping first {skip_shards} shards (post-shuffle); "
                  f"{len(self.txt_files) - skip_shards} remain, first is {self.txt_files[skip_shards].name}")
            self.txt_files = self.txt_files[skip_shards:]
...
def create_dataloader(
    ...
    skip_shards=0,  # Drop first N shards after the seeded shuffle
) -> DataLoader:
    """
    ...
    ds = ShardedPackedTextDataset(list(txt_files), tokenizer, seq_len=seq_len, cache_size=cache_size, shuffle=dataset_shuffle, num_shards=num_shards, seed=seed, skip_shards=skip_shards)
