...
# model/ only exists on the Greene deployment; the HF pretrained_model path never uses it
try:
...
# Greene-only module; used only when puzzle eval files are configured
try:
    from evaluation.multiturn_puzzle_evaluator import evaluate_multiturn_puzzle
except ImportError:
    evaluate_multiturn_puzzle = None
import torch
...
class SFTTrainer:
    ...
    def _init_all(self):
            ...
            if "block_size" in self.mcfg and self.mcfg["block_size"] is not None:
                ...
                if hasattr(self.model.config, "max_position_embeddings"):
                    ...
                    if bs > original_max_pos:
                        # capture the pretrain rope_theta BEFORE assigning rope_scaling:
                        # transformers 5.x maps that assignment onto rope_parameters,
                        # dropping the theta the checkpoint carried (1e6, not the 1e4
                        # class default) — needed again at export time
                        _rp = getattr(self.model.config, "rope_parameters", None)
                        self._pretrain_rope_theta = (
                            (_rp.get("rope_theta") if isinstance(_rp, dict) else None)
                            or getattr(self.model.config, "rope_theta", None)
                        )
        ...
        save_root = pathlib.Path(
            self.tcfg.get(
                "save_dir",
                "/scratch/js15262/LLM-Pretraining/checkpoints/sft_checkpoints",
            )
        )
    ...
    def _evaluate(self, max_steps: Optional[int] = 50) -> Dict[str, float]:
        ...
        if self.acc.is_main_process:
            ...
            human_eval_configs = [
                # {"evaluator": human_eval_opening, "file": "/scratch/js15262/LLM-Pretraining/data/chess/test/human_games_test_opening.parquet", "prefix": "human_opening"},
                # {"evaluator": human_eval_capture, "file": "/scratch/js15262/LLM-Pretraining/data/chess/test/human_games_test_capture.parquet", "prefix": "human_capture"},
                # {"evaluator": human_eval_random, "file": "/scratch/js15262/LLM-Pretraining/data/chess/test/human_games_test_random.parquet", "prefix": "human_random"},
                # {"evaluator": human_eval_promotion, "file": "/scratch/js15262/LLM-Pretraining/data/chess/test/human_games_test_promotion.parquet", "prefix": "human_promotion"},
                # {"evaluator": human_eval_check, "file": "/scratch/js15262/LLM-Pretraining/data/chess/test/human_games_test_check.parquet", "prefix": "human_check"},
                # {"evaluator": random_eval, "file": "/scratch/js15262/LLM-Pretraining/data/chess/test/random_games_100.parquet", "prefix": "random_games"},
            ]

            puzzle_files = [
                # {"file": "/scratch/js15262/LLM-Pretraining/data/chess/test/puzzles_grandmaster.csv", "prefix": "puzzles_grandmaster"},
                # {"file": "/scratch/js15262/LLM-Pretraining/data/chess/test/test_final.csv", "prefix": "test_final"},
                # {"file": "/scratch/js15262/chess_reasoning/datasets/puzzles_processed_2/puzzles_processed_new.csv", "prefix": "puzzles_train_new"},
            ]
    ...
    def _save_hf_model(self, path):
        ...
        if hasattr(base_model, 'config'):
            ...
            # Canonicalize rope fields in the exported config.json: write BOTH the
            # legacy old-transformers form (top-level rope_theta + rope_scaling) that
            # the miles/sglang RL stack reads, and the complete transformers-5.x form
            # (rope_parameters incl. rope_theta). Without this, new transformers drops
            # rope_theta from the export and downstream loaders fall back to 1e4.
            cfg_file = path / "config.json"
            if cfg_file.exists():
                with open(cfg_file) as f:
                    cj = json.load(f)
                rp = cj.get("rope_parameters") or cj.get("rope_scaling") or {}
                if rp.get("type", rp.get("rope_type")) == "yarn":
                    theta = (rp.get("rope_theta") or cj.get("rope_theta")
                             or getattr(self, "_pretrain_rope_theta", None) or 1000000)
                    base = {"factor": rp["factor"],
                            "original_max_position_embeddings": rp["original_max_position_embeddings"]}
                    cj["rope_theta"] = theta
                    cj["rope_scaling"] = {**base, "type": "yarn"}
                    cj["rope_parameters"] = {**base, "type": "yarn", "rope_type": "yarn",
                                             "rope_theta": theta}
                    with open(cfg_file, "w") as f:
                        json.dump(cj, f, indent=2)
                    self.acc.print(f"[SFT] Canonicalized rope config (theta={theta}, yarn x{rp['factor']})")
        else:
