...
def _scratch_model_family(model_cfg) -> str:
    """Resolve a supported from-scratch HF model family, failing closed."""
    family = model_cfg.get("model_family", model_cfg.get("model_type", None))
    if family is None:
    ...
    normalized = str(family or "").strip().lower().replace("-", "").replace("_", "")
    aliases = {
        "qwen": "qwen3",
        "qwen3": "qwen3",
        "llama": "llama",
    ...
    if normalized not in aliases:
        raise ValueError(
            "Unsupported from-scratch model family. Set model.model_family to "
            "'qwen3' or 'llama'; got "
            f"{family!r} (architecture={model_cfg.get('architecture', None)!r})."
        )
    return aliases[normalized]


def _plain_config_dict(value, *, field_name: str):
    """Convert an OmegaConf mapping to the plain dict expected by Transformers."""
    if value is None:
        return None
    try:
    ...
    if not isinstance(value, dict):
        raise TypeError(f"{field_name} must be a mapping or null, got {type(value).__name__}")
    return dict(value)


def _rope_config_kwargs(config_class, *, rope_theta: float, rope_scaling):
    """Bridge the official Transformers 4.x and 5.x RoPE config APIs."""
    import inspect
...
def _add_legacy_llama_rope_aliases(config_dict: dict) -> dict:
    """Keep Transformers 5.x Llama configs loadable with Transformers 4.x.

    Transformers 5 serializes the complete RoPE contract under
    ``rope_parameters``. Transformers 4 ignores that field and otherwise
    silently falls back to theta=10,000 with no scaling. Retain the 5.x field
    while also writing the equivalent legacy aliases.
    """
    if config_dict.get("model_type") != "llama":
        return config_dict
    rope_parameters = config_dict.get("rope_parameters")
    if not isinstance(rope_parameters, dict) or "rope_type" not in rope_parameters:
        return config_dict

    rope_theta = rope_parameters.get("rope_theta")
    if rope_theta is not None:
        config_dict["rope_theta"] = float(rope_theta)
...
def _build_scratch_hf_config(
    ...
    *,
    ...
    """Build an official Transformers config for a random-init model.

    Keeping this as a pure helper makes architecture selection testable without
    constructing dataloaders or an Accelerator. Existing Qwen configs continue
    to resolve from their ``Qwen/...`` architecture string; new families must
    be explicitly supported here instead of silently falling back to Qwen.
    """
    family = _scratch_model_family(model_cfg)
    hidden_size = int(model_cfg.n_embed)
    ...
    default_kv_heads = 4 if family == "qwen3" else num_attention_heads
    default_head_dim = 128 if family == "qwen3" else hidden_size // num_attention_heads
    num_key_value_heads = int(
        model_cfg.get("num_key_value_heads", default_kv_heads)
    )
    head_dim = int(model_cfg.get("head_dim", default_head_dim))
    ...
    if min(
        ...
        raise ValueError("All scratch-model dimensions and block_size must be positive")
    ...
    if family == "qwen3":
        ...
        rope_kwargs = _rope_config_kwargs(
            Qwen3Config,
            rope_theta=1000000,
            rope_scaling=None,
        )
        return Qwen3Config(
            # Preserve the established Qwen3 template used by existing runs.
            ...
            attention_dropout=float(model_cfg.get("dropout", 0.0)),
            **rope_kwargs,
        )
    ...
    max_position_embeddings = int(
        model_cfg.get("max_position_embeddings", block_size)
    )
    if max_position_embeddings < block_size:
        raise ValueError(
            "model.max_position_embeddings must cover model.block_size; got "
            f"{max_position_embeddings} < {block_size}"
        )
    rope_scaling = _plain_config_dict(
        model_cfg.get("rope_scaling", None),
        field_name="model.rope_scaling",
    )
    if rope_scaling is not None:
        original_max = rope_scaling.get("original_max_position_embeddings")
        if original_max is not None and int(original_max) >= max_position_embeddings:
            raise ValueError(
                "Llama RoPE original_max_position_embeddings must be smaller "
                "than max_position_embeddings; got "
                f"{original_max} >= {max_position_embeddings}"
            )
    ...
    return LlamaConfig(
        ...
        num_key_value_heads=num_key_value_heads,
        head_dim=head_dim,
        hidden_act=str(model_cfg.get("hidden_act", "silu")),
        ...
        attention_dropout=float(model_cfg.get("dropout", 0.0)),
        mlp_bias=bool(model_cfg.get("mlp_bias", False)),
...
class HFTrainer:
    ...
    def _init_all(self):
        ...
        self._skip_shards = int(self.dcfg.get("skip_shards", 0) or 0)
        ...
        if pretrained_model and not init_from_scratch:
            ...
            # model.load_dtype overrides the param dtype (e.g. float32 to keep fp32 master
            # weights + bf16 autocast, matching from-scratch Mode-3 runs, even when the
            # checkpoint on disk is bf16)
            load_dtype = self.mcfg.get("load_dtype", None)
            if load_dtype is not None:
                torch_dtype = getattr(torch, str(load_dtype))
            else:
            ...
            # Mode 3: Build a supported official HF architecture from scratch.
            family = _scratch_model_family(self.mcfg)
            self.acc.print(f"Initializing {family} model from scratch")
            ...
            self.model = AutoModelForCausalLM.from_config(hf_config, trust_remote_code=True)
            self.acc.print(
                f"[model] Created {type(self.model).__name__} from scratch "
                f"(family={family})"
            )
    ...
    @torch.no_grad()
    def _evaluate_kl_divergence(self, current_step: int, max_steps: int | None = None, stockfish_path: str = "/scratch/js15262/Stockfish/src/stockfish", stockfish_time: float = 0.1):
        """
    ...
    def _final_ckpt(self, step):
        ...
        # A daemon upload can be killed as soon as the training process exits.
        # Drain periodic uploads and make the final sync part of successful
        # training completion.
        self._upload_run_dir_to_hf("final", wait=True)

    def _upload_run_dir_to_hf(self, step_label, *, wait=False):
        """Upload the run directory, synchronously for the final checkpoint."""
        if not self.acc.is_main_process:
            return
        ...
        def _do_upload(*, raise_on_error=False):
            try:
                api = HfApi()
        ...
        upload_threads = getattr(self, "_hf_upload_threads", None)
        if upload_threads is None:
            upload_threads = []
            self._hf_upload_threads = upload_threads

        if wait:
            for thread in upload_threads:
                thread.join()
            upload_threads.clear()
            _do_upload(raise_on_error=True)
            return
